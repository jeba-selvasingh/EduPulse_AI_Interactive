import { ReadinessTierBoardPanel } from '@/src/components/ReadinessTierBoardPanel';
...
export default function ReadinessBoardScreen() {
  const router = useRouter();
  const allowed = usePermission(Permission.CampusDriveRead);

  if (!allowed) {
    return <AccessDenied featureName="Readiness tier board" />;
  }
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.phoneShell}>
        <View style={styles.hdr}>
          ...
          <Text style={styles.title}>Readiness tier board</Text>
        </View>

        <ScrollView contentContainerStyle={styles.body}>
          <ReadinessTierBoardPanel />
        </ScrollView>
      </View>
    </SafeAreaView>
  );
