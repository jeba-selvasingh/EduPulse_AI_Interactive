...
import { InterventionPriorityPanel } from '@/src/components/InterventionPriorityPanel';
...
export default function InterventionPriorityScreen() {
  const router = useRouter();
  const { focus } = useLocalSearchParams<{ focus?: string }>();
  const allowed = usePermission(Permission.CampusDriveRead);
  const focusValue = typeof focus === 'string' ? focus : undefined;

  if (!allowed) {
    return <AccessDenied featureName="Intervention priority list" />;
  }
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.phoneShell}>
        ...
        <ScrollView contentContainerStyle={styles.body}>
          <InterventionPriorityPanel focus={focusValue} />
        </ScrollView>
      </View>
    </SafeAreaView>
  );
