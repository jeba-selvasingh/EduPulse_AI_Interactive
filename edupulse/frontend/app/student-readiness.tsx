...
import { StudentReadinessDetailPanel } from '@/src/components/StudentReadinessDetailPanel';
...
export default function StudentReadinessScreen() {
  const router = useRouter();
  const { usn } = useLocalSearchParams<{ usn?: string }>();
  const allowed = usePermission(Permission.CampusDriveRead);
  const usnValue = typeof usn === 'string' ? usn : '';

  if (!allowed) {
    return <AccessDenied featureName="Student readiness detail" />;
  }
      ...
      <View style={styles.phoneShell}>
        <View style={styles.hdr}>
          ...
          <Text style={styles.title} numberOfLines={1}>
            {usnValue ? `Student · ${usnValue}` : 'Student readiness'}
          </Text>
        </View>

        <ScrollView contentContainerStyle={styles.body}>
          <StudentReadinessDetailPanel usn={usnValue} />
        </ScrollView>
      </View>
    </SafeAreaView>
  );
