...
import { DriveCalendarPanel } from '@/src/components/DriveCalendarPanel';
...
export default function DriveCalendarScreen() {
  const router = useRouter();
  const { driveId } = useLocalSearchParams<{ driveId?: string }>();
  const allowed = usePermission(Permission.CampusDriveRead);
  const driveValue = typeof driveId === 'string' ? driveId : undefined;
  const title = driveValue ? 'Drive details' : 'Drive calendar · Aug 2026';

  if (!allowed) {
    return <AccessDenied featureName="Drive calendar and reminders" />;
  }
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.phoneShell}>
        ...
        <ScrollView contentContainerStyle={styles.body}>
          <DriveCalendarPanel driveId={driveValue} />
        </ScrollView>
      </View>
    </SafeAreaView>
  );
